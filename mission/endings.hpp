	class End1
	{
		title = "Mission Complete";
		subtitle = "They will think twice about attacking us.";
		description = "Is that sand in my shoe ?";
		pictureBackground = "\a3\Ui_f\data\GUI\Cfg\Debriefing\debriefingBackground_co.paa";
		picture = "\a3\Ui_f\data\GUI\Cfg\Hints\Tasks_ca.paa";
		pictureColor[] = {1,1,1,1};
	};

	class End2
	{
		title = "Mission Failed";
		subtitle = "Game Over Man";
		description = "Too many allied casualties were taken.";
		pictureBackground = "\a3\Ui_f\data\GUI\Cfg\Debriefing\debriefingBackground_co.paa";
		picture = "\a3\Ui_f\data\GUI\Cfg\Hints\Incapacitated_ca.paa";
		pictureColor[] = {1,1,1,1};
	};
	
	class End3
	{
		title = "Mission Failed";
		subtitle = "Critical Asset Lost!";
		description = "You lost an asset critical to the completion of the mission, you have failed!";
		pictureBackground = "\a3\Ui_f\data\GUI\Cfg\Debriefing\debriefingBackground_co.paa";
		picture = "\a3\Ui_f\data\GUI\Cfg\Debriefing\endDefault_ca.paa";
		pictureColor[] = {1,1,1,1};
	};
	
	class End4
	{
		title = "Partial Completion";
		subtitle = "You did some things";
		description = "But it was not enough, the mission was not completed.";
		pictureBackground = "\a3\Ui_f\data\GUI\Cfg\Debriefing\debriefingBackground_co.paa";
		picture = "\a3\Ui_f\data\GUI\Cfg\Hints\Rules_ca.paa";
		pictureColor[] = {1,1,1,1};
	};
		
	class End5
	{
		title = "Mission Over";
		subtitle = "Something bad happened";
		description = "The mission is deemed a failure.";
		pictureBackground = "\a3\Ui_f\data\GUI\Cfg\Debriefing\debriefingBackground_co.paa";
		picture = "\a3\Ui_f\data\GUI\Cfg\Debriefing\endDeath_ca.paa";
		pictureColor[] = {1,1,1,1};
	};
	
	class End6
	{
		title = "Out of Time";
		subtitle = "You failed to complete your Objectives within the required time.";
		description = "The mission is a failure.";
		pictureBackground = "\a3\Ui_f\data\GUI\Cfg\Debriefing\debriefingBackground_co.paa";
		picture = "\a3\Ui_f\data\GUI\Cfg\Hints\Fatigue_ca.paa";
		pictureColor[] = {1,1,1,1};
	};